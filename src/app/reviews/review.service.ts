...
export class ReviewService {
    ...
    getReviews(): Observable<IReview[]> {
        return this._http.get<IReview[]>(this._reviewUrl)
            .do(data => {})
    ...
    getReview(id: string): Observable<IReview> {
      return this._http.get<IReview[]>(this._reviewUrl + '/' + id)
          .do(data => {})
          .catch(this.handleError);
    }
    ...
    editReview(review: IReview): Observable<any> {
      return this._http.put(this._reviewUrl + '/' + review.uniqueId, JSON.stringify(review), {
        headers: new HttpHeaders().set('Content-Type', 'application/json'),
      })
      .map(this.extractData)
      .catch(this.handleError);
